...
/**
* $Id: metagen.php,v 1.3 2005/04/19 18:21:12 fx2024 Exp $
* Module: SmartClient
* Author: The SmartFactory <www.smartfactory.ca>
* Licence: GNU
*/

function smartclient_metagen_html2text($document)
{
...
function smartclient_createMetaDescription($description, $maxWords = 100)
{
	$myts =& MyTextSanitizer::getInstance();
	
	$words = array();
	$words = explode(" ", smartclient_metagen_html2text($description));
...
function smartclient_findMetaKeywords($text, $minChar)
{
	$myts =& MyTextSanitizer::getInstance();
...
function smartclient_createMetaTags($title, $categoryPath='', $description = '', $minChar=4)
{
	...
	If (isset($title) && ($title != '')) {
		$keywords = smartclient_findMetaKeywords($title, $minChar);
	...
	If ($description != '') {
		$xoopsTpl->assign('xoops_meta_description', smartclient_createMetaDescription($description));	
	}
